...
app.use('/api/private', (req: AllowedUser, res: Response, next: NextFunction) => {
    try {
        verifyUser(req);
        next();
    } catch (error) {
...
app.use('/api/private/verifyinitialaccess', (req: AllowedUser, res: Response) => {
    try {
        verifyUser(req);
...
const verifyUser = (req: AllowedUser) => {
    const token: string = req.headers.cookie?.split('=')[1] ?? '';
    if (!token) throw new Error('Undefined token');
    const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as Id;
    req.user = decoded;
}
